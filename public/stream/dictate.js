(function (window) {
  ...
  var Dictate = function (cfg) {
    ...
    config.onSubmitUserInput = config.onSubmitUserInput || function () { };
    config.onMicToggle = config.onMicToggle || function () { };
    config.language = config.language || DEFAULT_LANGUAGE;
    config.botVoice = config.botVoice;
    ...
    this.init = function () {
      if (!config.botVoice) {
        // If botVoice is false, return early to avoid asking for microphone access
        config.onEvent(
          MSG_INIT_RECORDER,
          "botVoice is false, skipping microphone access."
        );
        return;
      }
    ...
    this.sendEOFToServer = function (buffer) {
      socketSend(buffer || TAG_END_OF_AUDIO);
    }
    ...
    function createWebSocket() {
      ...
      ws.onmessage = function (e) {
        ...
        if (data instanceof Object && !(data instanceof Blob)) {
          ...
          if (res.success == true) {
            ...
            let replacedText = res.display_text.replace(' amps ', ' MPs ');
            
            let stt_tag= res.id;
            window.updateResponseId(stt_tag);

            // replacedText = updatePanNumberFormat(replacedText);

            // remove <unknown> from the text
            replacedText = replacedText.replace(/<unknown>/g, '');

            // remove space from the end of the text
            replacedText = replacedText.replace(/ $/g, '');

            // remove . from the end of the text only if the last character is "."
            if (replacedText[replacedText.length - 1] === '.') {
              replacedText = replacedText.replace(/\.$/g, '');
            }

            if (res.cause == 'partial') {
              config.onPartialResults(replacedText);
              config.onEvent(MSG_WEB_SOCKET, JSON.stringify({ display_text: replacedText }));
            } else if (res.cause == 'silence detected' || res.cause == 'EOF received' || res.cause == 'timeout') {
              console.info(JSON.stringify({ res }));
              let text = replacedText;
              config.onResults(text);
              config.onEvent(MSG_WEB_SOCKET, JSON.stringify({ display_text: replacedText }));
            }
            ...
            // 	If the result is final then the transcripted data should get submitted
            // const isFinalOutput = res?.final;
            // if (isFinalOutput) {
            // 	config.onSubmitUserInput();
            //
            // 	// Need to close the websocket connection otherwise next time recording will not work
            // 	if (ws) {
            // 		ws.close();
            // 		ws = null;
            // 	}
      ...
      ws.onopen = function (e) {
        ...
        // 		}
        // }
        // socketSend(JSON.stringify(stt_config));
